...
function RealtimeChart({
  ...
  useEffect(() => {
    if(chartDeviation.current != null){
    ...
    (chartValue.current as unknown as HTMLElement).innerHTML = data.datasets[0].data?.[0] ?? 0;
    if (diff < 0) {
    ...
    chartDeviation.current.innerHTML = `${diff > 0 ? '+' : ''}${isNaN(diff) ? "" : diff?.toFixed(2)+"%" }`;
  }
