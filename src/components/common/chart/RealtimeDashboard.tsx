...
function RealtimeDashboard({charData = [], unit, title}: Props) {
  ...
  const chartData = {
    labels: charData?.map(cd => cd.label),
    datasets: [
      // Indigo line
      {
        data:  charData?.map(cd => cd.value),
        fill: true,
  ...
  return (
    <div className="flex flex-col col-span-full sm:col-span-6 bg-white shadow-lg rounded-sm border border-gray-200">
      <header className="px-5 py-4 border-b border-gray-100 flex items-center">
        <h2 className="font-semibold text-gray-800">{title ??"Real Time Stream"}</h2>
      ...
      <RealtimeChart data={chartData} width={595} height={248} unit={unit} />
    </div>
  );
}
